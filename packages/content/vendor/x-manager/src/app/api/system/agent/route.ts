  ...
  return NextResponse.json({
    ...
    coreEndpoints: [
      ...
      },
      {
        ...
        description: 'List or create automation rules for event, schedule, and keyword triggers.',
      },
      {
        id: 'automation_rule_update',
        method: 'PATCH/DELETE',
        path: '/api/automation/rules/:id',
        description: 'Update, enable/disable, or delete an automation rule.',
      },
      {
        id: 'automation_rule_log',
        method: 'GET',
        path: '/api/automation/rules/:id/log',
        description: 'List execution history for an automation rule.',
      },
      {
        id: 'feeds',
        method: 'GET/POST',
        path: '/api/feeds',
        description: 'List or register RSS/Atom feeds for monitoring and optional auto-scheduling.',
      },
      {
        id: 'feed_update',
        method: 'PATCH/DELETE',
        path: '/api/feeds/:id',
        description: 'Update or remove an RSS feed monitor.',
      },
      {
        id: 'feed_entries',
        method: 'GET',
        path: '/api/feeds/:id/entries',
        description: 'List processed entries for a monitored feed.',
      },
      {
        id: 'saved_searches',
        method: 'GET/POST',
        path: '/api/discovery/saved',
        description: 'List or create saved keyword searches with optional auto-actions.',
      },
      {
        id: 'saved_search_update',
        method: 'PATCH/DELETE',
        path: '/api/discovery/saved/:id',
        description: 'Update or remove a saved keyword search.',
      },
      {
        id: 'url_shorten',
        method: 'POST',
        path: '/api/urls',
        contentType: 'application/json',
        description: 'Create a short URL with optional UTM params and post association.',
        fields: [
          { name: 'target_url', required: true },
          { name: 'utm_source', required: false },
          { name: 'utm_medium', required: false },
          { name: 'utm_campaign', required: false },
          { name: 'post_id', required: false },
        ],
      },
      {
        id: 'url_list',
      ...
      },
      {
        ...
        method: 'GET',
        path: '/api/urls/:code',
        description: 'Get click analytics for a short URL.',
      },
      {
        id: 'url_redirect',
        method: 'GET',
        path: '/api/r/:code',
        description: '301 redirect with click tracking and UTM injection.',
      },
      {
        id: 'follower_timeseries',
        method: 'GET',
        path: '/api/analytics/followers',
        description: 'Follower count timeseries. Supports ?account_slot and ?days params.',
      },
      {
        id: 'follower_growth',
        method: 'GET',
        path: '/api/analytics/followers/growth',
        description: 'Daily follower growth rate with delta and average.',
      },
      {
        id: 'analytics_export',
        method: 'GET',
      ...
      {
        id: 'openclaw_bridge_batch',
        method: 'POST',
        path: '/api/bridge/openclaw/batch',
        contentType: 'application/json',
        description:
          'Batch bridge endpoint for publishing multiple posts in one call. Returns per-item results. Max 10 posts per batch. Same auth as single bridge endpoint.',
        fields: [
          { name: 'posts', required: true, note: 'Array of post objects (max 10).' },
          { name: 'posts[].text|content|message', required: true, note: 'Tweet text (max 280 chars).' },
          { name: 'posts[].account|account_slot|slot', required: false, default: 1, note: 'Account slot or username.' },
          { name: 'posts[].media_urls', required: false, note: 'Up to 4 image URLs per post.' },
          { name: 'posts[].community_id', required: false },
          { name: 'posts[].reply_to_tweet_id', required: false },
          { name: 'dryRun|dry_run|simulate', required: false, default: false },
        ],
      },
      {
        id: 'agent_webhooks',
        ...
        contentType: 'application/json',
        description:
          'Register, list, and delete webhook subscriptions. Webhooks fire on events like post.published, task.completed, approval.decided, etc.',
        fields: [
          { name: 'url', required: true, note: 'POST only. Webhook delivery URL.' },
          { name: 'events', required: true, note: 'POST only. Array of event types to subscribe to (or ["*"] for all).' },
          { name: 'secret', required: false, note: 'POST only. HMAC-SHA256 signing secret for delivery verification.' },
          { name: 'description', required: false, note: 'POST only. Human-readable label.' },
          { name: 'id', required: true, note: 'DELETE only. Webhook ID as ?id=N query param.' },
        ],
      },
    ],
    curlExamples: {
      ...
      openclawBridgeBatch: `curl -sS -X POST ${baseUrl}/api/bridge/openclaw/batch \\\n  -H "Authorization: Bearer $OPENCLAW_BRIDGE_TOKEN" \\\n  -H 'Content-Type: application/json' \\\n  -d '{\n    "posts": [\n      {"text": "First post", "account": "swarm_signal"},\n      {"text": "Second post", "account": "swarm_signal"}\n    ],\n    "dryRun": true\n  }'`,
      listWebhooks: `curl -sS ${baseUrl}/api/agent/webhooks`,
      registerWebhook: `curl -sS -X POST ${baseUrl}/api/agent/webhooks \\\n  -H 'Content-Type: application/json' \\\n  -d '{"url":"https://agent.example.com/notify","events":["post.published","task.completed","approval.decided"],"secret":"your-hmac-secret"}'`,
      deleteWebhook: `curl -sS -X DELETE '${baseUrl}/api/agent/webhooks?id=1'`,
    },
    errorCodes: [
      ...
      { code: 'MEDIA_UPLOAD_FAILED', status: 502, description: 'Failed to upload media to X.' },
      { code: 'APPROVAL_REQUIRED', status: 202, description: 'Action requires human approval before execution.' },
      { code: 'VALIDATION_ERROR', status: 400, description: 'Request body failed validation. Check fields property.' },
      { code: 'BRIDGE_NOT_CONFIGURED', status: 503, description: 'Bridge token is not configured.' },
      { code: 'UNAUTHORIZED', status: 401, description: 'Invalid or missing authentication.' },
    ...
    notes: [
      ...
      'All error responses include a machine-readable "code" field for programmatic branching. See errorCodes above.',
      'Global rate limit: 120 requests/minute per IP across all API endpoints (429 + Retry-After header).',
      'Register webhooks via POST /api/agent/webhooks to receive real-time event notifications instead of polling.',
      'Batch bridge supports up to 10 posts per call with per-item results and error isolation.',
